...
const WeatherMap = () => {
    ...
    const handleSearch = () => {
        const results = weatherData.filter(item => item.district.toLowerCase() === searchInput.toLowerCase());
        setSearchResults(results);
    };

    const handleResultClose = (district) => {
        // Logic to remove the result with the specified district from the searchResults array
        const updatedResults = searchResults.filter(result => result.district !== district);
                ...
                <div className="searchcon">
                    ...
                    <div className="search-card">
                        {searchResults.map(result => (
                            <div key={result.district} className="search-result-card">
                                <div className="result-header">
                                    <h3>{result.district}</h3>
                                    <button className="close-button" onClick={() => handleResultClose(result.district)}>Close</button>
                                </div>
